...
S_hat_test_NHMM_DF <- matrix(nrow = R, ncol = (test_size*T))
...
S_Replicas <- matrix(nrow = R, ncol = (T))
Y_Replicas <- matrix(nrow = R, ncol = (T))
...
sigma = c(3.5,2.5,6) #Vetor com os desvios padrões para as duas normais
...
for (p in 1:R){
  ...
  if (inherits(arima_mod,"try-error")){
    if (p == 1){
      Y_hat_test_arima <- rnorm(length(S_test), mean(mu_hat), sum(sigma_hat))
      print("Error in ARIMA Replica 1")
    } else {
  ...
  S_hat_test_NHMM_DF[p,] <- S_hat_test
...
y4 <- Y_hat_test_glmnet_DF[6,]

plot(x, y1, type = 'l', col = 'blue', ylim = c(min(y1, y2), max(y1, y2)), 
     xlab = 'X-axis', ylab = 'Y-axis', main = 'GLMNET vs ARIMA vs NHMM-LASSO vs Real', lwd=1.25)
lines(x, y2, col = 'red', lty=2, lwd=2)
lines(x, y3, col = 'green')
lines(x, y4, col = 'black')

legend('topright', legend = c('LASSO-NHMM', 'ARIMA', 'GLMNET'),inset=c(-1, 0), col = c('blue', 'red', 'green'), xpd = TRUE)
