...
K=2 #Quantidade de estados ocultos
...
for (z in 1:length(lambdas)){
  for (p in 1:length(lambdas)){
    ...
    options(digits=8)
    options(scipen=999)
    cat(paste("\nValores de Lambda: ","(",toString(lambdas[z]),",",toString(lambdas[p]),")", collapse = "")) #Messagem indicando o numero da replica atual
...
for (z in 1:length(lambdas)){
  for (p in 1:length(lambdas)){
    lambda_labels[label_gen] <- paste("(",toString(lambdas[z]),",",toString(lambdas[p]),")", sep = "")  
...
y_temp<-NULL
z_temp<-NULL
z_t = matrix(nrow = length(lambdas), ncol = length(lambdas))

counter = 1
for (z in 1:length(lambdas)){
  for (p in 1:length(lambdas)){
    x_temp[counter] = lambdas[z]
    y_temp[counter] = lambdas[p]
    z_temp[counter] = avg_Accuracy[counter]
    z_t[z,p] = avg_Accuracy[counter]
    counter = counter + 1
  }
}
...
#Funciona#
library("plot3D")
scatter3D(x_temp, y_temp, z_temp, colvar = NULL, bty = "b2", col = "blue", type = "h", 
          pch = 19, cex = 0.5, theta = 20, phi = 15, ticktype = "detailed")
...
grid <- mesh(x, y)
z    <- with(grid, z_t)
persp3D(z = z, x = x, y = y, theta = 20, phi = 15,
...
library(plotly)
fig <- plot_ly(x = x_temp, y = y_temp, z = z_temp, type = 'mesh3d')
