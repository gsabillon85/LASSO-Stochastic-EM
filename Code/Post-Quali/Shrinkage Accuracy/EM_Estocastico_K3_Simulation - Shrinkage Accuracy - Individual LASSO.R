...
T=200 #Cumprimento da cadeia simulada
...
lambdas <- c(0, 0.0001, 0.001, 0.01, 0.1, 0.25, 0.5, 1, 2.5, 5, 10)
...
avg_RMSE = NULL
sd_RMSE = NULL
avg_Parameters = matrix(nrow = length(lambdas)^K, ncol = D*K*(K-1) )
...
for (u in 1:length(lambdas)){
  for (z in 1:length(lambdas)){
    for (p in 1:length(lambdas)){
      ...
      options(digits=8)
      options(scipen=999)
      cat(paste("\nValores de Lambda: ","(",toString(lambdas[u]),",",toString(lambdas[z]),",",toString(lambdas[p]),")", collapse = "")) #Messagem indicando o numero da replica atual
      for (h in 1:length(seeds)){
        ...
        FSM1 <-function(params){#função a maximizar para achar os Betas_1
          resp <- sum(1 - log(1 + exp(Xtemp11%*%params[1:D])+ exp(Xtemp11%*%params[(D+1):(2*D)]))) + sum((Xtemp12%*%params[1:D]) - log( 1 + exp(Xtemp12%*%params[1:D])+ exp(Xtemp12%*%params[(D+1):(2*D)]) )) + sum((Xtemp13%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp13%*%params[1:D])+ exp(Xtemp13%*%params[(D+1):(2*D)]) )) - lambda1*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)]))) #lambda*(abs(params[2]) + abs(params[3]) + abs(params[5]) + abs(params[6]))  #lambda*sum(abs(params[1:2*D])) 
        }
        
        FSM2 <-function(params){#função a maximizar para achar os Betas_2
          resp <- sum(1 - log(1 + exp(Xtemp21%*%params[1:D])+ exp(Xtemp21%*%params[(D+1):(2*D)]))) + sum((Xtemp22%*%params[1:D]) - log( 1 + exp(Xtemp22%*%params[1:D])+ exp(Xtemp22%*%params[(D+1):(2*D)]) )) + sum((Xtemp23%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp23%*%params[1:D])+ exp(Xtemp23%*%params[(D+1):(2*D)]) )) - lambda2*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)])))#lambda*(abs(params[2]) + abs(params[3]) + abs(params[5]) + abs(params[6])) #lambda*sum(abs(params[1:2*D])) 
        }
        
        FSM3 <-function(params){#função a maximizar para achar os Betas_3
          resp <- sum(1 - log(1 + exp(Xtemp31%*%params[1:D])+ exp(Xtemp31%*%params[(D+1):(2*D)]))) + sum((Xtemp32%*%params[1:D]) - log( 1 + exp(Xtemp32%*%params[1:D])+ exp(Xtemp32%*%params[(D+1):(2*D)]) )) + sum((Xtemp33%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp33%*%params[1:D])+ exp(Xtemp33%*%params[(D+1):(2*D)]) )) - lambda3*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)])))#lambda*(abs(params[2]) + abs(params[3]) + abs(params[5]) + abs(params[6]))  #lambda*sum(abs(params[1:2*D])) 
        }
      ...
      avg_RMSE[lambda_counter] = mean(RMSE)
      sd_RMSE[lambda_counter] = sd(RMSE)
      avg_Sensitivity[lambda_counter] = mean(Sensitivity)
      sd_Sensitivity[lambda_counter] = sd(Sensitivity)
      avg_Specificity[lambda_counter] = mean(Specificity)
      sd_Specificity[lambda_counter] = sd(Specificity)
      avg_Accuracy[lambda_counter] = mean(Accuracy)
      sd_Accuracy[lambda_counter] = sd(Accuracy)
      avg_pctgm_zerado[lambda_counter] = mean(pctgm_zerado)
      sd_pctgm_zerado[lambda_counter] = sd(pctgm_zerado)
      for (o in 1:(D*K*(K-1))){
        avg_Parameters[lambda_counter,o] <- mean(Parameters[,o])
      }
...
for (u in 1:length(lambdas)){
  for (z in 1:length(lambdas)){
    for (p in 1:length(lambdas)){
      lambda_labels[label_gen] <- paste("(",toString(lambdas[u]),",",toString(lambdas[z]),",",toString(lambdas[p]),")", sep = "")  
...
df_avgs<-data.frame(cbind(lambda_labels, avg_Sensitivity, avg_Specificity, avg_Accuracy, avg_pctgm_zerado, avg_RMSE))
df_sd <- data.frame(cbind(lambda_labels, sd_Sensitivity, sd_Specificity, sd_Accuracy, sd_pctgm_zerado, sd_RMSE))
df_full <- data.frame(cbind(lambda_labels, avg_Sensitivity, sd_Sensitivity, avg_Specificity, sd_Specificity, avg_Accuracy, sd_Accuracy, avg_pctgm_zerado, sd_pctgm_zerado, avg_RMSE, sd_RMSE))
