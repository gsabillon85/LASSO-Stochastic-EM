...
T=3000 #Cumprimento da cadeia simulada
K=2   #Numero de estados ocultos
D=10   #Quantidade de Covariaveis
R<-2 #Numero de Replicas que serão executadas
...
folder_name = paste("/",toString(D),"covs_Output_EM-Estocastico_GroupedLASSO_","K",toString(K),"_","T",toString(T), sep = "")
...
for (p in 1:R) {
  ...
  lambdas = seq(log(exp(0.000001)), log(10000000000), len=80)
  ...
  mu_teste_vector = matrix(nrow = length(lambdas)^2, ncol = K)
  sigma_teste_vector = matrix(nrow = length(lambdas)^2, ncol = K)
  Beta1_teste_vector = matrix(nrow = length(lambdas)^2, ncol = D)
  Beta2_teste_vector = matrix(nrow = length(lambdas)^2, ncol = D)
  Y_hat_teste_vector = matrix(nrow = length(lambdas)^2, ncol = T*test_size)
  acertos_teste <- rep.int(0,length(lambdas)^2)
  EQM_Teste = NULL
  ...
  for (lasso_iter1 in 1:length(lambdas)){
    for (lasso_iter in 1:length(lambdas)){
      ...
      #  resp <- ((sum(1 - log(1 + exp(Xtemp11%*%params[1:10]))) + sum(Xtemp12%*%params[1:10] - log(1 + exp(Xtemp12%*%params[1:10])))) - lambdas[lasso_iter1]*sum(abs(params[2:10]))) + ((sum(1 - log(1 + exp(Xtemp21%*%params[11:20]))) + sum(Xtemp22%*%params[11:20] - log(1 + exp(Xtemp22%*%params[11:20]))))  - lambdas[lasso_iter]*sum(abs(params[12:20])))
      #}
      
      FSM1 <-function(params){#função a maximizar para achar os Betas_1
        resp <- ( (sum(1 - log(1 + exp(Xtemp11%*%params[1:10]))) + sum(Xtemp12%*%params[1:10] - log(1 + exp(Xtemp12%*%params[1:10])))) ) + ( (sum(1 - log(1 + exp(Xtemp21%*%params[11:20]))) + sum(Xtemp22%*%params[11:20] - log(1 + exp(Xtemp22%*%params[11:20])))) ) - ( lambdas[lasso_iter1]*sum(abs(params[2:10])) + lambdas[lasso_iter]*sum(abs(params[12:20])) )
      }
        ...
        for (i in 1:K){
          for (d in 1:D){
            if (i == 1){
              ...
              BetaArray[i,d,1]=fit1$par[1:10][d]
            }
        ...
        for (i in 1:K){
          for (d in 1:D){
            if (i == 1){
              ...
              BetaArray[i,d,2]=fit1$par[11:20][d]
            }
        ...
        for (i in 1:K){
          for (j in 1:D){
            if (i == 1){
              ...
              BetaArray[i,j,1]=fit1$par[1:10][j]
            }
        ...
        for (i in 1:K){
          for (j in 1:D){
            if (i == 1){
              ...
              BetaArray[i,j,2]=fit1$par[11:20][j]
            }
  ...
  acertos_teste[1:length(lambdas)^2]/length(S_teste)
  ############################################################
  ...
  # 
  # Betas_Rep[p,4] = BetaArray[2,1,2]
  # Betas_Rep[p,5] = BetaArray[2,2,2]
  # Betas_Rep[p,6] = BetaArray[2,3,2]
