...
Mat_trans <-function(covar){
  ...
  for (j in 1:K) {
    for (i in 1:K){
      B[i,j] = exp(covar%*%BetaArray[i,,j])/(exp(covar%*%BetaArray[1,,j])+exp(covar%*%BetaArray[2,,j]))
    }  
  }
  return(B)
}
...
folder_name = paste("/",toString(D),"covs_Output_EM-Estocastico_IndividualLASSO_","K",toString(K),"_","T",toString(T), sep = "")
...
AICc_Acum<-NULL
BIC_Acum<-NULL
Betas_Rep<-matrix(nrow = R, ncol = 6)
mu_Rep<-matrix(nrow = R, ncol = 2)
sigma_Rep<-matrix(nrow = R, ncol = 2)
acertos<-rep.int(0,R)
...
Vies_Table<-matrix(nrow = 10, ncol = R)
CritInfo_Table<-matrix(nrow = 2, ncol = R)

P0 = rep(1/K,K) #Inicializamos vetor de probabilidades inciais para o HMM
mu = c(30,80) # vetor com media para as duas Normais
sigma = c(1,3) #Vetor com os desvios padrões para as duas normais
...
qtd_nonzero_coefs = 3
for (i in 1:K){
  for (j in (qtd_nonzero_coefs+1):D){
    Betas[2,j,i] = 0
  }
}
...
for (p in 1:R) {
  ...
  TransCount <- matrix(data = c(0,0,0,0), nrow = K, ncol = K) #matriz que almacena a quantidade de vezes que cada transição ocorre
  ...
  init1 = c(c(rnorm(D, 3, 3)))#Valores iniciais para os Betas_1 
  init2 = c(c(rnorm(D, 3, 3)))#Valores iniciais para os Betas_2
  
  
  
  #Antes de Rodar o EM Estocástico, precisaremos de algumas estruturas para
  #almacenar os valores de mu_hat, sigma_hat, S_treino
  lambdas = seq(log(exp(0.001)), log(10000000000), len=499)
  ...
  mu_teste_vector = matrix(nrow = length(lambdas), ncol = K)
  sigma_teste_vector = matrix(nrow = length(lambdas), ncol = K)
  Beta1_teste_vector = matrix(nrow = length(lambdas), ncol = D)
  Beta2_teste_vector = matrix(nrow = length(lambdas), ncol = D)
  Y_hat_teste_vector = matrix(nrow = length(lambdas), ncol = T*test_size)
  acertos_teste <- rep.int(0,length(lambdas))
  EQM_Teste = NULL
  # O EMEst sera rodado em duas etapas. Uma para estimar 
  # S e theta, e outra  para estimar os Betas
  loop_count=1
  print('Running LASSO')
  for (lasso_iter in 1:length(lambdas)){
    ...
    FSM1 <-function(params){#função a maximizar para achar os Betas_1
      resp <- (sum(1 - log(1 + exp(Xtemp11%*%params))) + sum(Xtemp12%*%params - log(1 + exp(Xtemp12%*%params)))) - lambdas[lasso_iter]*sum(abs(params[2:D])) 
    }
    
    FSM2 <-function(params){#função a maximizar para achar os Betas_2
      resp <- (sum(1 - log(1 + exp(Xtemp21%*%params))) + sum(Xtemp22%*%params - log(1 + exp(Xtemp22%*%params))))  - lambdas[lasso_iter]*sum(abs(params[2:D]))
    }
    ...
    while (tolval[val]>tol){
      ...
      TransCount <- matrix(data = c(0,0,0,0), nrow = K, ncol = K)
      for (i in 2:length(S_training)) {
        for (j in 1:K) {
          for (k in 1:K) {
      ...
      for (j in 1:K) {
        for (k in 1:K) {
          if (TransCount[k,j]==0){
            positions = sample(2:length(S_training), 4)
            for (d in 1:4) {
    ...
    perms_S = ecr(zpivot = zpvt_S, z = mat_S, K = 2)# aplicamos o metodo ECR que retornara as permutações das dos estados ocultos que devem ser utilizadas para reordenar a saida do algoritmo bayesiano
    ...
    Beta1_teste_vector[loop_count, ] = BetaArray[2,,1]
    Beta2_teste_vector[loop_count, ] = BetaArray[2,,2]
  ...
  acertos_teste[1:length(lambdas)]/length(S_teste)
  ############################################################
  ...
  Beta1_teste_vector[1,]
  Beta2_teste_vector[1,]
  index_min = which.min(EQM_Teste)
  index_min
  acertos_S_min_teste = acertos_teste[index_min]/length(Y_teste)
  Betas_teste1 = Beta1_teste_vector[index_min,]
  Betas_teste2 = Beta2_teste_vector[index_min,]
  EQM_Teste[index_min]
  
  #Marcamos quais coeficientes foram zerados pelo LASSO
  for (d in 1:D){
    if(abs(Betas_teste1[d]) >= 0.02){
  ...
  mu_Rep[p,1] = mu_teste_vector[index_min,1]
  mu_Rep[p,2] = mu_teste_vector[index_min,2]
  
  sigma_Rep[p,1] = sigma_teste_vector[index_min,1]
  sigma_Rep[p,2] = sigma_teste_vector[index_min,2]
  
  
  Vies_Table[7,p]<-mu[1]-mu_hat[1]
  Vies_Table[8,p]<-mu[2]-mu_hat[2]
  Vies_Table[9,p]<-sigma[1]-sigma_hat[1]
  Vies_Table[10,p]<-sigma[2]-sigma_hat[2]
  ...
  Tempo[p]<-(tempo_final - tempo_inicial)/60
  
}# Fim das Replicas

acertos_teste/length(S_teste)
#Inicializamos algumas variaveis para
#almacenar calculos finais importantes

plot(lambdas, EQM_Teste)

acertos_teste[index_min]/length(S_teste)
#########################################################
...
quantiles<-matrix(nrow = 10, ncol = 2)
...
quantiles[7,]<-quantile(mu_Rep[,1],probs = c(0.025, 0.975))
quantiles[8,]<-quantile(mu_Rep[,2],probs = c(0.025, 0.975))
quantiles[9,]<-quantile(sigma_Rep[,1],probs = c(0.025, 0.975))
quantiles[10,]<-quantile(sigma_Rep[,2],probs = c(0.025, 0.975))
...
IC_95<-c(toString(quantiles[7,]),toString(quantiles[8,]),toString(quantiles[9,]),toString(quantiles[10,]))
...
EQM_Min_Final_Median = median(na.omit(EQM_min_LASSO_Rep))
EQM_Min_Final = mean(na.omit(EQM_min_LASSO_Rep))
EQM_Min_SD = sd(na.omit(EQM_min_LASSO_Rep))

#Media e DP do EQM medio obtido ao longo do Lasso em cada Replica
EQM_Mean_LASSO_Final = mean(na.omit(EQM_mean_LASSO_Rep))
EQM_Mean_LASSO_Final_Median = median(na.omit(EQM_mean_LASSO_Rep))
EQM_Mean_LASSO_SD = sd(na.omit(EQM_mean_LASSO_Rep))

#Media e DP do EQM para modelo sem Lasso 
EQM_No_LASSO_Final = mean(na.omit(EQM_No_LASSO_Rep))
EQM_No_LASSO_Final_Median = median(na.omit(EQM_No_LASSO_Rep))
EQM_No_LASSO_SD = sd(na.omit(EQM_No_LASSO_Rep))
...
saveWorkbook(wb, nome_arquivo)#guardamos o arquivo de ex
...
mean(na.omit(EQM_No_LASSO_Rep))
median(na.omit(EQM_No_LASSO_Rep))
sd(na.omit(EQM_No_LASSO_Rep))

mean(na.omit(EQM_mean_LASSO_Rep))
median(na.omit(EQM_mean_LASSO_Rep))
sd(na.omit(EQM_mean_LASSO_Rep))
