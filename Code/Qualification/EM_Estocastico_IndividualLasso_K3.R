...
for (p in 1:R){
  ...
  Beta21_teste_vector = matrix(nrow = length(lambdas), ncol = D)
  Beta22_teste_vector = matrix(nrow = length(lambdas), ncol = D)
  Beta23_teste_vector = matrix(nrow = length(lambdas), ncol = D)
  Beta31_teste_vector = matrix(nrow = length(lambdas), ncol = D)
  Beta32_teste_vector = matrix(nrow = length(lambdas), ncol = D)
  Beta33_teste_vector = matrix(nrow = length(lambdas), ncol = D)
  ...
  for (lasso_iter in 1:length(lambdas)){
    ...
    FSM1 <-function(params){#função a maximizar para achar os Betas_1
      resp <- (sum(1 - log(1 + exp(Xtemp11%*%params[1:D]) + exp(Xtemp11%*%params[(D+1):(2*D)]))) + sum(Xtemp12%*%params[1:D] - log(1 + exp(Xtemp12%*%params[1:D]) + exp(Xtemp12%*%params[(D+1):(2*D)]))) + sum(Xtemp13%*%params[(D+1):(2*D)] - log(1 + exp(Xtemp13%*%params[1:D]) + exp(Xtemp13%*%params[(D+1):(2*D)])))) - (lambdas[lasso_iter]*sum(abs(params[2:D])) + lambdas[lasso_iter]*sum(abs(params[(D+2):(2*D)]))) 
    }
    
    FSM2 <-function(params){#função a maximizar para achar os Betas_2
      resp <- (sum(1 - log(1 + exp(Xtemp21%*%params[1:D]) + exp(Xtemp21%*%params[(D+1):(2*D)]))) + sum(Xtemp22%*%params[1:D] - log(1 + exp(Xtemp22%*%params[1:D]) + exp(Xtemp22%*%params[(D+1):(2*D)]))) + sum(Xtemp23%*%params[(D+1):(2*D)] - log(1 + exp(Xtemp23%*%params[1:D]) + exp(Xtemp23%*%params[(D+1):(2*D)])))) - (lambdas[lasso_iter]*sum(abs(params[2:D])) + lambdas[lasso_iter]*sum(abs(params[(D+2):(2*D)]))) 
    }
    
    FSM3 <-function(params){#função a maximizar para achar os Betas_2
      resp <- (sum(1 - log(1 + exp(Xtemp31%*%params[1:D]) + exp(Xtemp31%*%params[(D+1):(2*D)]))) + sum(Xtemp32%*%params[1:D] - log(1 + exp(Xtemp32%*%params[1:D]) + exp(Xtemp32%*%params[(D+1):(2*D)]))) + sum(Xtemp33%*%params[(D+1):(2*D)] - log(1 + exp(Xtemp33%*%params[1:D]) + exp(Xtemp33%*%params[(D+1):(2*D)])))) - (lambdas[lasso_iter]*sum(abs(params[2:D])) + lambdas[lasso_iter]*sum(abs(params[(D+2):(2*D)]))) 
    }
