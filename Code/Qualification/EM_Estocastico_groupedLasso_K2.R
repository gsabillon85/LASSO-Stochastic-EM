...
for (p in 1:R) {
  ...
  lambdas = seq(log(exp(0.001)), log(10000000000), len=50)
  ...
  for (lasso_iter1 in 1:length(lambdas)){
    for (lasso_iter in 1:length(lambdas)){
      ...
      FSM1 <-function(params){#função a maximizar para achar os Betas_1
        resp <- (sum(1 - log(1 + exp(Xtemp11%*%params))) + sum(Xtemp12%*%params - log(1 + exp(Xtemp12%*%params)))) - lambdas[lasso_iter1]*sum(abs(params[2:D])) 
      }
